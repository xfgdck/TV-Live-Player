    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-fade-in">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {/* TAP 3: MANAGE CURRENT SUBSCRIPTIONS */}
          {activeTab === "manage" && (
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-neutral-300">已包含的播放源列表</h3>
              {customSources.length === 0 ? (
                <div className="text-center py-12 bg-neutral-950/50 rounded-2xl border border-neutral-800 border-dashed">
                  <Bookmark className="w-12 h-12 text-neutral-600 mx-auto mb-3" />
                  <p className="text-neutral-400 text-sm">当前没有任何自定义导入的 M3U 源</p>
                  <p className="text-neutral-500 text-xs mt-1">您可以试着通过第一个选项卡添加精品推荐源！</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {customSources.map((src) => (
                    ...
                    >
                      <div className="space-y-1 max-w-[80%]">
                        ...
                        <p className="text-xs text-neutral-500 truncate font-mono" title={src.url}>
                          {src.url}
                        </p>
                        <p className="text-[10px] text-neutral-600 font-mono">
                          导入时间: {new Date(src.createdAt).toLocaleString()}
                        </p>
                      </div>

                      <button
                        id={`btn_delete_source_${src.id}`}
                        onClick={() => onRemovePlaylist(src.url)}
